...
class RomFSEntry(RomFSCommon):
    ...
    COMPRESSED_MASK = 0x005B0000 # This is probably wrong
    ...
    def get_data(self, uid):
        ...
        try:
            ...
            try:
                ...
                if len(data) != self.entries[uid].size_decompressed:
                    warn("[zlib] Wrong decompressed size! %s (%d)" % (repr(self.entries[uid]), uid))
            except KeyboardInterrupt as e:
                raise e
            except Exception as e:
                if 0 != self.entries[uid].size_decompressed:
...
if __name__ == '__main__':
    ...
    try:
        ...
        print ("Usage: %s <input romfs.bin> <output directory>" % sys.argv[0])
        sys.exit(1)
    ...
    for (uid, info) in fs.entries.items():
        if hasattr(info, 'name') and hasattr(info, 'parent'):
            ...
            if info.type == "directory" and not os.path.exists(fname):
                os.makedirs(fname)
                dirs_ctimes.append((fname, info.ctime))
            else:
                fdata = fs.get_data(uid)
                with open(fname, 'wb') as fp:
                    fp.write(fdata)
                try:
                    os.utime(fname, times=(info.ctime, info.ctime))
                except KeyboardInterrupt as e:
                    raise e
                except Exception as e:
                    pass
    ...
    try:
        for dirname, ctime in dirs_ctimes:
            os.utime(dirname, times=(ctime, ctime))
    except KeyboardInterrupt as e:
        raise e
    except Exception as e:
        pass
